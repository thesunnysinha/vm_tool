...
class SetupRunnerConfig(BaseModel):
    ...
    github_project_url: Optional[str] = Field(
        default=None,
        description="URL of the GitHub repository (required for setup/clone operations)",
    )
...
class SetupRunner:
    ...
    def _require_github_url(self) -> str:
        """Raise a clear error if github_project_url was not provided."""
        if not self.github_project_url:
            raise ValueError(
                "github_project_url is required for this operation. "
                "Pass it via SetupRunnerConfig(github_project_url='https://github.com/...')."
            )
        return self.github_project_url
    ...
    def _run_ansible_playbook(
        ...
        sensitive_env: Optional[dict] = None,
    ):
        """Executes an Ansible playbook. Delegates to AnsibleRunner.

        Args:
            extravars: Non-sensitive variables to pass to the playbook.
            ...
            sensitive_env: Sensitive values (tokens, passwords) passed as env vars,
                           not logged by ansible-runner.
        """
    ...
    def run_setup(self):
        ...
        sensitive_env = {
            "VM_TOOL_GITHUB_TOKEN": self.github_token or "",
            "VM_TOOL_DOCKERHUB_PASSWORD": self.dockerhub_password or "",
        }
        ...
        self._run_ansible_playbook(extravars, inventory_file="inventory.yml", sensitive_env=sensitive_env)

    def run_cloud_setup(self, ssh_configs: List[SSHConfig]):
        """Runs the cloud setup using Ansible with dynamic inventory generation.

        Per-host SSH passwords are written as ansible_ssh_pass directly into the
        per-host inventory entry. The inventory file is a 0o600 temp file cleaned
        up in a finally block, so passwords are never stored long-term.
        Key-based auth (ssh_identity_file) is preferred when available.
        """
        self._require_github_url()
        ...
        for i, ssh_config in enumerate(ssh_configs):
            ...
            if ssh_config.ssh_identity_file:
                ...
                # Written to 0o600 temp file cleaned up in finally — not persisted
        ...
        try:
            with tempfile.NamedTemporaryFile(
                mode="w", suffix=".yml", prefix="vm_tool_inv_", delete=False
            ) as tmp_inv:
                yaml.dump(inventory_content, tmp_inv)
                inventory_file_path = tmp_inv.name
            os.chmod(inventory_file_path, 0o600)
            ...
            if inventory_file_path and os.path.exists(inventory_file_path):
                try:
    ...
    def run_k8s_deploy(
        ...
        generated_inventory_path = None

        try:
            # Handle base64-encoded kubeconfig (CI pipelines)
            if kubeconfig_b64:
                decoded = base64.b64decode(kubeconfig_b64)
                # Create with 0o600 from the start — no race window
                kc_fd = tempfile.mkstemp(suffix=".kubeconfig", prefix="vm_tool_kc_")
                os.chmod(kc_fd[1], 0o600)
                with os.fdopen(kc_fd[0], "wb") as kc_f:
                    kc_f.write(decoded)
                ...
                temp_kubeconfig = _TmpFile(kc_fd[1])
            ...
            with tempfile.NamedTemporaryFile(
                ...
                yaml.dump(inventory_content, tmp_inv)
                generated_inventory_path = tmp_inv.name
            os.chmod(generated_inventory_path, 0o600)
            ...
            if generated_inventory_path and os.path.exists(generated_inventory_path):
                try:
                    os.unlink(generated_inventory_path)
                except OSError:
                    pass
    ...
    def run_docker_deploy(
        ...
        try:
            self._run_ansible_playbook(
                extravars,
                playbook="push_code.yml",
                inventory_file=target_inventory,
                sensitive_env=sensitive_env,
            )

            logger.info("Docker deployment completed successfully.")
            if host:
                state.record_deployment(host, compose_file, compose_hash, service_name)
    ...
    def setup_ci_environment(provider="github"):
        ...
        # 2. Create Inventory
        # Per-run known_hosts file: accepts new hosts on first connect,
        # rejects changed host keys on subsequent connects (MITM protection)
        known_hosts_path = f"{key_path}.known_hosts"
