...
class CanaryDeployment:
    ...
    def _deploy_to_canary(self, compose_file: str):
        """Deploy to canary host using Docker Compose."""
    ...
    def _shift_traffic(self, percentage: int):
        ...
        raise NotImplementedError(
            f"_shift_traffic must be overridden for your load balancer. "
            f"Target canary: {self.canary_host} at {percentage}% traffic. "
...
class ProgressiveRollout:
    ...
    def __init__(self, hosts: List[str], batch_size: int = 1, ssh_user: Optional[str] = None):
        self.hosts = hosts
    ...
    def _deploy_to_host(self, host: str, compose_file: str):
        """Deploy to a single host using Docker Compose."""
        from vm_tool.core.runner import SetupRunner, SetupRunnerConfig
        SetupRunner(SetupRunnerConfig()).run_docker_deploy(
            compose_file=compose_file,
            host=host,
